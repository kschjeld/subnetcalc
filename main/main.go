...
func main() {
	...
	//
	//fmt.Printf("\n\nReservations:\n")
	//reservations := s.Collect(subnetcalc.SelectReserved())
	...
	//
	//s, err = subnetcalc.Parse("10.0.0.0/16")
	//size16 := s.Collect(subnetcalc.SelectWithSize(17))
	//for _, r := range size16 {
	//	fmt.Printf(" %s\n", r.CIDR())
	//}
	//
	//fmt.Printf("\n\n\n")
	...
	if sn, err := s.FindFree(18); err == nil && s != nil {
		sn.Reserve("test1")
	}
	...
	if sn, err := s.FindFree(19); err == nil && s != nil {
		sn.Reserve("test3")
	}
	...
	fmt.Printf("Reserved:\n")
	found := s.Collect(subnetcalc.SelectReserved())
	for _, r := range found {
		fmt.Printf("-  %s (%s - %s)\n", r.CIDR(), r.FirstIP(), r.LastIP())
	}

	fmt.Printf("Available:\n")
	found = s.Collect(subnetcalc.SelectWithSize(18), subnetcalc.SelectAvailable())
	for _, r := range found {
		fmt.Printf("-  %s (%s - %s)\n", r.CIDR(), r.FirstIP(), r.LastIP())
	}
